...
export function getMediaPoster(movieName: string | null, movieReleaseDate: number | null): string | undefined {
  if (movieReleaseDate && movieName) {
    const promise = getImage<Array<any>>(`https://www.omdbapi.com/?apikey=daf26042&t=${movieName}&y=${movieReleaseDate}`);
  ...
  if (movieName) {
    const promise = getImage<Array<any>>(`https://www.omdbapi.com/?apikey=daf26042&t=${movieName}`);
