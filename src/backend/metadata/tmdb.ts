...
export function getMediaPoster<T>(movieName: string | null, movieReleaseDate: number | null): string | undefined {
  if (movieReleaseDate && movieName) {
    const promise = getImage<{data: {PromiseState: boolean, PromiseResult: {Poster: string}}}>(`https://www.omdbapi.com/?apikey=daf26042&t=${movieName}&y=${movieReleaseDate}`);
  ...
  if (movieName) {
    const promise = getImage<{data: {PromiseState: boolean, PromiseResult: {Poster: string}}}>(`https://www.omdbapi.com/?apikey=daf26042&t=${movieName}`);
